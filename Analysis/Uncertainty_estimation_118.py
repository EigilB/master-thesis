...
#model = keras.models.load_model('/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/Keras/Big1_best_acc74')
#
#
...
#torch.save(net, '/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/net.pt')
...
bnn = torch.load('/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/BNN_best_acc287.pt')
...
#
#model = keras.models.load_model('/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/Keras/Big_BNN1_best_acc87')
#model = keras.models.load_model('/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/9bus/MC_dropout_best_acc46.h5')
#W_list = model.weights
#for i in range(len(W_list)):
#    W_list[i] = W_list[i].numpy()
#
#hyperparameters  = pd.read_csv('/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/Keras/Big_BNN1_RandomSearch87.csv', delimiter=",")
#hyperparameters2 = hyperparameters.iloc[0]
#hyperparameters3 = hyperparameters2.values.squeeze().tolist()
#batch_size, alpha, lr_decay, nl, n1, n2, n3, n4, n5, do_p1, do_p2, do_p3, do_p4, do_p5, _, _ = hyperparameters3
#hyperparameters_dict = {'batch_size' : batch_size, 'alpha' : alpha, 'lr_decay' : lr_decay, 'nl' : nl,\
#                        'n1' : n1, 'n2' : n2, 'n3' : n3, 'n4' : n4, 'n5' : n5,'do_p1' : do_p1, 'do_p2' : do_p2,\
#                        'do_p3' : do_p3, 'do_p4' : do_p4, 'do_p5' : do_p5}
#
#hyperparameters_dict['dim'] = dim
#
#net_do = Net_dropout(hyperparameters_dict)
#net_do.load_weights_w_bias(W_list)
#torch.save(net_do, '/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/net_do.pt')
net_do = torch.load('/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/net_do.pt')
...
for i in range(1, 20+1):
    model = keras.models.load_model('/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/Keras/Ensembles/model'+str(i)+'.h5')
    W_list = model.weights
    architechture = [W_list[0].shape[0]]
...
#for i in range(1, 20+1):
#    torch.save(ensembles[i], '/Users/Eigil/Dropbox/DTU/Speciale/Analysis/Models/118bus/Ensembles/net'+str(i)+'.pt')
...
y_pred_prob_net_val = net.forward_P(X_val_torch).detach().numpy().squeeze()#Validation probability
...
y_pred_prob_bnn_val = n_forward_passes(bnn, X_val_torch, no_forward_passes=1000, return_matrix=False)
...
y_pred_prob_netdo_val = n_forward_passes(net_do, X_val_torch, no_forward_passes=1000, verbose=False, return_matrix=False)
...
y_pred_prob_ens_val = ensemble_forward_pass(ensembles, X_val_torch, return_matrix=False)
...
plt.title('ROC plot for the 118-bus models')
plt.tight_layout()
...
print('BNN')
print(confusion_matrix(y_test, y_pred_bnn))
print('MC-Dropout')
print(confusion_matrix(y_test, y_pred_netdo))
print('Deep Ensemble')
...
for j in range(4):
    ...
    acc, Bn, conf = bin_acc(y_pred_prob, y_test.squeeze(), M)
    ...
    acc, Bn, conf = bin_acc2(y_pred_prob, y_test.squeeze(), M)
...
for j in range(4): 
    ...
    for i, T in enumerate(T_list):
        ...
        ECE = bin_acc(y_pred_prob_T, y_val, M=15, ECE_calc=True)
        MCE = bin_acc(y_pred_prob_T, y_val, M=15, MCE_calc=True)
        ECE_matrix[i, j] = ECE
        MCE_matrix[i, j] = MCE
...
for i in range(len(prob_list_full)):
    ...
    ECE = bin_acc(y_pred_prob, y_test, M=15, ECE_calc=True)
    MCE = bin_acc(y_pred_prob, y_test, M=15, MCE_calc=True)
    BS = brier_score_loss(y_test, y_pred_prob)
    NLL = nll(y_pred_prob, y_test)
...
#metrics_df.to_csv('/Users/Eigil/Dropbox/DTU/Speciale/Saved tables/Uncertainty_metrics_118bus.csv', sep=",")
...
for j in range(4):
    ...
    correct_pred = (y_pred.squeeze()[index] == y_test.squeeze()[index]).astype(int) #Prediction vector
    TP_vector = ((y_pred[index] == 1) & (y_test.squeeze()[index] == 1)).astype(int) 
    FN_vector = ((y_pred[index] == 0) & (y_test.squeeze()[index] == 1)).astype(int) 
    FP_vector = ((y_pred[index] == 1) & (y_test.squeeze()[index] == 0)).astype(int) 
    TN_vector = ((y_pred[index] == 1) & (y_test.squeeze()[index] == 1)).astype(int)
...
no_samples = 10000
X_outliers2 = np.random.uniform(low=-1, high=2, size=(no_samples, dim))
...
ax[0].set_title('Using entropy as OOD detector')
ax[1].set_title('Using disagreement as OOD detector')
plt.tight_layout()
