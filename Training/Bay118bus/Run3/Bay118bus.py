par_number = 1
import sys
best_indices = [4,  16,  28,  54,  57,  84, 109, 111, 117, 124, 149, 151, 157,\
       169, 174, 181, 183, 185, 189, 198, 206, 219, 238, 244, 249, 258,\
       270, 276, 285, 286, 287, 301, 303, 318, 322, 324, 336, 342, 346,\
       353, 375, 380, 383, 384, 395, 398, 404, 413, 420, 424, 437, 439,\
       447, 455, 457, 465, 471, 481, 485, 497, 501, 506, 513, 532, 555,\
       593, 596, 597, 646, 649, 672, 678, 687, 716, 721, 732, 739, 759,\
       772, 778, 788, 794, 795, 810, 814, 822, 824, 832, 839, 861, 871,\
       872, 929, 936, 937, 956, 961, 966, 972, 980]

if par_number not in best_indices:
	print('Not in best indices')
	sys.exit(0)
       

Result_dir = '/zhome/13/e/97883/Documents/Speciale_code/Results/Bay118bus/Run3/'
data_dir = '/zhome/13/e/97883/Documents/Speciale_code/Data/BigCombined1.mat'
past_models_dir = '/zhome/13/e/97883/Documents/Speciale_code/Results/Bay118bus/Run_1_and_2/'
...
batch_size, alpha, nl, n1, n2, n3, n4, pi, sigma1, sigma2, draws, accuracy, ECE, MCE = A.tolist()
...
for i in range(no_samples):
    ...
    bnn = torch.load(past_models_dir+'best_acc'+str(par_number))
    ...
    #Validation accuracy
    y_pred_prob = n_forward_passes(bnn, torch.from_numpy(X_val), no_forward_passes = 1000, verbose=False)
    y_pred = (y_pred_prob > 0.5).astype(int)
    
    acc = np.mean(np.mean(y_pred.squeeze() == y_val.squeeze())) #97.9%
    ECE = bin_acc(y_pred_prob, y_val, M=15, ECE_calc=True)
    MCE = bin_acc(y_pred_prob, y_val, M=15, MCE_calc=True)
